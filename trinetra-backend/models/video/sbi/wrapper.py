...
Source: https://github.com/mapooon/SelfBlendedImages
        Also available via DeepfakeBench

SBI trains an XceptionNet to detect blending boundaries along jawlines and
around eyes that appear in face-swap deepfakes.

Input:  raw video bytes or a single face image.
Output: probability that the image/video contains face-swap artifacts.

Exposes:
  GET  /health   → {"status": "ok", "model": "sbi", "weight_version": "..."}
...
class XceptionSBI(nn.Module):
    """XceptionNet used by SBI for blending artifact detection."""
    def __init__(self, nb_classes=2):
        ...
        self.last_linear = nn.Linear(2048, nb_classes)
...
def _extract_frames(file_bytes: bytes, n_frames: int = 8) -> List[Image.Image]:
    ...
    try:
        subprocess.run(
            ["ffmpeg", "-i", video_path, "-vf", "fps=3,scale=299:299",
             "-frames:v", str(n_frames), "-q:v", "2", f"{out_dir}/frame_%04d.jpg"],
            capture_output=True, timeout=30,
        )
        images = [Image.open(f).convert("RGB") for f in sorted(Path(out_dir).glob("*.jpg"))[:n_frames]]
    except Exception:
        images = []
    finally:
...
class SBIWrapper(BaseModelWrapper):
    MODEL_NAME = "sbi"
    WEIGHTS_PATH = Path("weights/sbi_xception.pth")

    def _load_model(self) -> None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self._device = device
        model = XceptionSBI(nb_classes=2)
        ...
        if weights_path.exists():
            ...
            print("[SBI] Loaded pretrained XceptionNet weights.")
        else:
            print("[SBI] WARNING: weights not found. Running in dev mode.")
    ...
    def _run_inference(self, file_bytes: bytes) -> float:
        ...
        is_video = (file_bytes[4:8] == b"ftyp") or file_bytes[:3] == b"ID3"
